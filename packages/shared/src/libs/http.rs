use config::{Config, models::{ProxyConfig, ProxyStrategy}};
...
impl ProxyRotator {
    ...
    pub fn init() -> SoundomeResult<()> {
        ...
        GLOBAL_PROXY_ROTATOR.set(rotator).map_err(|_| {
            Error::Config("Failed to set global proxy rotator".into())
        })
    }
    ...
    pub fn get() -> &'static Self {
        GLOBAL_PROXY_ROTATOR.get().expect("ProxyRotator is not initialized")
    }
    ...
    pub fn get_next_proxy(&self) -> Option<String> {
        if let Some(config) = &self.config {
            ...
            tracing::debug!("Selected proxy URL: {} (strategy: {:?})", url, config.strategy);
            Some(url)
        } else {
            None
        }
            ...
            if let Some(no_proxy_list) = &config.no_proxy {
                for no_proxy_domain in no_proxy_list {
                    if domain.contains(no_proxy_domain) {
                        tracing::debug!("Domain {} excluded from proxy due to no_proxy rule: {}", domain, no_proxy_domain);
                        return false;
                    }
                }
            }
...
impl HttpClientBuilder {
    ...
    pub fn get_reqwest_client() -> SoundomeResult<reqwest::Client> {
        Self::get_reqwest_client_builder()?.build().map_err(|e| {
            Error::Network(format!("Failed to build HTTP client: {}", e))
        })
    }

    /// Creates a reqwest client with a specific proxy
    pub fn get_reqwest_client_with_specific_proxy(proxy_url: Option<&str>) -> SoundomeResult<reqwest::Client> {
        let mut client_builder = reqwest::Client::builder();
    ...
    fn create_reqwest_proxy(proxy_url: &str) -> SoundomeResult<reqwest::Proxy> {
        if proxy_url.starts_with("socks5://") {
            ...
            reqwest::Proxy::all(proxy_url)
                .map_err(|e| Error::Config(format!("Invalid SOCKS5 proxy URL '{}': {}", proxy_url, e)))
        } else {
            // For HTTP/HTTPS proxies
            reqwest::Proxy::all(proxy_url)
                .map_err(|e| Error::Config(format!("Invalid HTTP proxy URL '{}': {}", proxy_url, e)))
        }
    }
    ...
    fn parse_proxy_url(input: &str) -> String {
        ...
        if input.starts_with("http://") || input.starts_with("https://") || input.starts_with("socks5://") {
