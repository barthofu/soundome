...
use rustypipe::{client::RustyPipe, model::{MusicSearchResult, TrackItem}};
use shared::{errors::Error, models::{album::Album, artist::Artist, track::{Track, TrackProvider}}};
...
impl YoutubeMusic {
    ...
    fn convert_search_item_to_track(&self, search_item: TrackItem) -> Track {
        Track {
            ...
            artists: search_item.artists.iter().map(|artist| Artist {
            ...
            provider_url: Some(format!("https://music.youtube.com/watch?v={}", search_item.id)),
            source: None,
            source_url: None,
            date: None,
            cover: None,
...
impl Provider for YoutubeMusic {
    ...
    async fn search(&self, track: &Track) -> Result<String, Error> {
        ...
        let best_match = self.match_results(
            search_results.items.items.iter().map(|search_item| self.convert_search_item_to_track(search_item.clone())).collect(),
            track.clone()
