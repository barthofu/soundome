...
use openrouter_api::{ChatCompletionRequest, JsonSchema, Message, OpenRouterClient, ProviderPreferences, ResponseFormat, Unconfigured};
...
async fn openrouter(prompt: &str, api_key: &str) -> Result<String, Error> {
    ...
    let chat_api = client.chat()
        .map_err(|err| Error::Custom(err.to_string()))?;
    chat_api.chat_completion(request).await
        .map(|response| {
            // println!("{:#?}", response);
...
async fn main() {
    ...
    let tracks = client.get_playlist_tracks(ResourceId::Url("https://soundcloud.com/bartho-az/sets/euphoria-part-4".to_string())).await.unwrap();
    let tracks = tracks.iter().map(|t| Track {
...
//     let processed_tracks = openrouter.generate_with_data(&prompt, tracks).await.unwrap();
    let processed_tracks = ai_client.generate_with_data(ai::prompts::CLEAN_TRACK_TITLE_AND_ARTIST_NAME, tracks).await.unwrap();
